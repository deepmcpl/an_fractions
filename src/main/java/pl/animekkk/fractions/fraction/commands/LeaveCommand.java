...
public class LeaveCommand extends Command {
    ...
    public boolean onExecute(CommandSender commandSender, String[] args) {
        ...
        if(fraction.getOwner() == user.getUuid()) {
            if(args.length < 1) return ChatUtils.sendMessage(player,
                ...
                ChatUtils.sendMessage(player, "&7You have deleted your fraction.");
            }
        } else {
            fraction.removeMember(user.getUuid());
            user.setFraction(null);
            ChatUtils.sendMessage(player, "&7You have left &3" + fraction.getTag() + " &7fraction.");
        }
        return true;
    }
}
