...
public class JoinCommand extends Command {
    ...
    public boolean onExecute(CommandSender commandSender, String[] args) {
        ...
        if(fraction != null) return ChatUtils.sendMessage(player, "&7You already belong to &3" + fraction.getTag() + " &7fraction.");
        if(args.length != 1) return ChatUtils.sendMessage(player, "&7Wrong usage. (&3/join <tag>&7)");
        Fraction newFraction = FractionManager.getFraction(args[0]);
        if(newFraction == null) return ChatUtils.sendMessage(player, "&7This fraction does not exist.");
        if(!newFraction.getInvites().contains(user.getUuid())) return ChatUtils.sendMessage(player, "&7You have not been invited to this fraction.");
        user.setFraction(newFraction);
        newFraction.removeInvite(user.getUuid());
        newFraction.getMembers().add(user.getUuid());
        return ChatUtils.sendMessage(player, "&7You have joined &3" + newFraction.getTag() + " &7fraction.");
    }
}
